...
namespace ConAnecoop
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            CargaBloqueo(connectionString, usuario, cooperativa, password, fechaInicio, fechaFin, campanya, informa);
        ...
        public static void CargaOriginal(string connectionString, string usuario, string cooperativa, string password, DateTime fechaInicio, DateTime fechaFin, string campanya, bool informa)
        {
        ...
        public static void CargaBloqueo(string connectionString, string usuario, string cooperativa, string password, DateTime fechaInicio, DateTime fechaFin, string campanya, bool informa)
        {
            ...
            foreach (AnecoopConsultaExpediente.JSVExpedienteXmlExpediente exp in res)
            {
                ...
                decimal importeBloqueado = 0;
                decimal importeBloqueadoPago = 0;
                ...
                // Primero mirar si el registro se encuentra en la base de datos y si está bloqueado
                string sql = "select * from anecoop where expediente_id='{0}' and linea_expediente='{1}' and codigo_campanya='{2}' and bloqueado = TRUE";
                sql = String.Format(sql, expediente_id, linea_expediente, codigo_campanya);
                cmd.CommandText = sql;
                ...
                if (rdr.HasRows)
                {
                    bloqueado = true;
                    rdr.Read();
                    importeBloqueado = rdr.GetDecimal("importe_liq");
                }
                else
                {
                    bloqueado = false;
                ...
                if (!bloqueado)
                {
                    ...
                    // VRS 1.0.0.2
                    // Se ha eliminado del update "numero_salida_cooperativa='{11}', "
                    sql = @"insert into anecoop(expediente_id, pdls_id, linea_expediente, codigo_campanya, 
                    periodo, codigo_cooperativa_gestora, nombre_gestora, codigo_cooperativa_carga, 
                    nombre_carga, pto_carga, nombre_pto_carga, numero_salida_cooperativa, 
                    nlinea_salida_cooperativa, n_pedido_aneccop, n_pedido, n_linea,
                    tipo_expediente, estado_coop_expediente, expediente_reenviado, codigo_delegacion,
                ...
                else
                {
                    ...
                    if (importeBloqueado == 0)
                    {
                        ...
                        sql = @"UPDATE anecoop
                            ...
                            codigo_tipo_articulo='{45}', codigo_cliente='{46}', nombre_cliente='{47}', codigo_pais='{48}', codigo_divisa_cliente='{49}',
                            codigo_producto='{50}', nombre_producto='{51}', codigo_calibre='{52}', nombre_calibre='{53}', npalet='{54}',
                            tipo_palet='{55}', nombre_tipo_palet='{56}', unidad_peso='{59}', precio_comercial='{60}',
                            codigo_divisa='{61}', liquidado_son='{62}', estado_coop_liquidacion='{63}', liquidacion_agrupada_son='{64}', codigo_grupo_liquidacion='{65}',
                            num_liq='{66}', fra_liq='{67}', fra_liq_complementa='{68}', carta_liq='{69}', registro_liq='{70}', importe_liq='{71}', importe_iva_liq='{72}', tipo_iva_liq='{73}', 
                            fecha_liq='{74:yyyy-MM-dd}', fecha_cambio_liq='{75:yyyy-MM-dd}', cambio_liq='{76}', fecha_sc_liq='{77:yyyy-MM-dd}', porcent_iva_liq='{78}', importe2_iva_liq='{79}', cobrado_son='{80}',
                            pagado_son='{81}', valor_mercancia='{82}', valor_confeccion='{83}', numero_salida_anecoop='{84}', salidalineaid='{85}'
                            WHERE expediente_id = '{0}' and linea_expediente = '{2}' and codigo_campanya = '{3}'
                            ";
                    }
                    else
                    {
                        // Ya tiene importe (los importes no los actualizamos)
                        sql = @"UPDATE anecoop
                            SET
                            pdls_id='{1}',
                            periodo='{4}', codigo_cooperativa_gestora='{5}', nombre_gestora='{6}', codigo_cooperativa_carga='{7}', 
                            ...
                            tipo_palet='{55}', nombre_tipo_palet='{56}', unidad_peso='{59}', precio_comercial='{60}',
                            codigo_divisa='{61}', liquidado_son='{62}', estado_coop_liquidacion='{63}', liquidacion_agrupada_son='{64}', codigo_grupo_liquidacion='{65}',
                            num_liq='{66}', fra_liq='{67}', fra_liq_complementa='{68}', carta_liq='{69}', registro_liq='{70}', tipo_iva_liq='{73}', 
                            fecha_liq='{74:yyyy-MM-dd}', fecha_cambio_liq='{75:yyyy-MM-dd}', cambio_liq='{76}', fecha_sc_liq='{77:yyyy-MM-dd}', porcent_iva_liq='{78}', importe2_iva_liq='{79}', cobrado_son='{80}',
                            pagado_son='{81}', valor_mercancia='{82}', valor_confeccion='{83}', numero_salida_anecoop='{84}', salidalineaid='{85}'
                            WHERE expediente_id = '{0}' and linea_expediente = '{2}' and codigo_campanya = '{3}'
                            ";
                ...
                if (pagos != null)
                {
                    foreach (AnecoopConsultaExpediente.JSVExpedienteXmlPago pago in pagos)
                    {
                        ...
                        {
                            // cabecera estaba bloqueado, verificamos el importe de pago para ver si lo actualizamos o no
                            importeBloqueadoPago = 0;
                            string sql2 = "select * from anecoop_pago where expediente_id='{0}' and expediente_pagoid='{1}'";
                            sql2 = String.Format(sql2, expediente_id, expediente_pagoid);
                            ...
                            if (rdr2.HasRows)
                            {
                                bloqueado = true;
                                rdr2.Read();
                                importeBloqueadoPago = rdr2.GetDecimal("importe");
                            }
                            rdr2.Close();

                            if (importeBloqueadoPago == 0)
                            {
                                // nos comportamos igual que si el registro no estuviera bloqueado
                                sql = @"INSERT INTO anecoop_pago(expediente_id,expediente_pagoid,tipo_pago,num_factura,fecha_factura,num_liquidacion,importe,fecha_pago,estado) 
                                values ('{0}','{1}','{2}','{3}','{4:yyyy-MM-dd}','{5}','{6}','{7:yyyy-MM-dd}','{8}')
                                ON DUPLICATE KEY UPDATE
                                tipo_pago='{2}',num_factura='{3}',fecha_factura='{4:yyyy-MM-dd}',num_liquidacion='{5}',importe='{6}',fecha_pago='{7:yyyy-MM-dd}',estado='{8}'
                            ...
                            {
                                // actualizamos todo menos el importe
                                sql = @"UPDATE anecoop_pago
                                SET
                                tipo_pago='{2}',num_factura='{3}',fecha_factura='{4:yyyy-MM-dd}',num_liquidacion='{5}',fecha_pago='{7:yyyy-MM-dd}',estado='{8}'
                                WHERE expediente_id='{0}' and expediente_pagoid='{1}'    
                                ";
                            }
                        }
                        sql = String.Format(sql, expediente_id, expediente_pagoid, tipo_pago, num_factura, fecha_factura, num_liquidacion, importe.ToString().Replace(",", "."), fecha_pago, estado);
